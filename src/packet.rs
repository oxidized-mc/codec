...
//!
//! Designed around a single trait + unified codec error to enable generic packet handling.
...
// ---------------------------------------------------------------------------
// ResourceLocationError (inlined from types to break circular dependency)
// ---------------------------------------------------------------------------

/// Errors that can occur when constructing or parsing a resource location.
///
/// Defined here in the codec crate (rather than in mc-types) so that
/// [`PacketDecodeError`] can hold it without creating a circular dependency.
/// The `mc-types` crate re-exports this type alongside [`ResourceLocation`].
///
/// [`ResourceLocation`]: https://docs.rs/oxidized-mc-types/latest/oxidized_mc_types/struct.ResourceLocation.html
#[derive(Debug, Error)]
#[non_exhaustive]
